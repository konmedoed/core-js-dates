...
function getNextFriday(date) {
  ...
  switch (new Date(date).getDay()) {
    case 0:
      return new Date(Date.parse(date) + day * 5);
    case 1:
      return new Date(Date.parse(date) + day * 4);
    case 2:
      return new Date(Date.parse(date) + day * 3);
    case 3:
      return new Date(Date.parse(date) + day * 2);
    case 4:
      return new Date(Date.parse(date) + day);
    case 5:
      return new Date(Date.parse(date) + day * 7);
    case 6:
      return new Date(Date.parse(date) + day * 6);
    default:
      return date;
  }
}
...
function getCountDaysInMonth(month, year) {
  const previousDay = Date.parse(new Date(year, month)) - 86400000;
  return new Date(previousDay).getDate();
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const timeBetween = Date.parse(dateEnd) - Date.parse(dateStart);
  return Math.floor(timeBetween / 86400000) + 1;
}
...
function formatDate(date) {
  ...
  const hour =
    new Date(date).getUTCHours() % 12 === 0
  ...
  const meridiem = new Date(date).getUTCHours() / 12 < 1 ? 'AM' : 'PM';
...
function getCountWeekendsInMonth(month, year) {
  const days = new Date(Date.parse(new Date(year, month)) - 86400000).getDate();
  let summ = 0;
  for (let i = 1; i <= days; i += 1) {
    const day = new Date(year, month - 1, i).getDay();
    if (day === 0 || day === 6) summ += 1;
  }
  return summ;
}
...
function getWeekNumberByDate(/* date */) {
  ...
  // for (let i = 1; i <= date.getMonth(); i += 1) {
  //   days += new Date(Date.parse(new Date(year, i)) - 86400000).getDate();
  // }
  // if (new Date(year, 0, 1).getDay() !== 1 && ()) return Math.ceil(days / 7) + 1;
  // if (new Date(year, 0, 1).getDay() >= date.getDay())
...
function getNextFridayThe13th(date) {
  ...
  let nextFriday;
  function checkInLastMonths() {
    for (let i = month; i < 12; i += 1) {
      if (new Date(year, i, 13).getDay() === 5) {
        nextFriday = new Date(year, i, 13);
        break;
      }
    }
  }
...
function isLeapYear(date) {
  const year = date.getFullYear();
  const feb = new Date(Date.parse(new Date(year, 2, 1)) - 86400000);
